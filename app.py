...
# ---- Calculate KPIs ----
# Handle empty dataset after all filters
if df_filtered.empty:
...
avg_shipment_time = df_filtered["Shipment Time"].mean()
...
kpi_data = [
    {"title": "Total Sales Revenue", "value": f"${format_number(total_sales)}"},
    {"title": "Average Order Value", "value": f"${format_number(avg_order_value)}"},
    {"title": "Total Orders Placed", "value": f"{format_number(total_orders)}"},
    {"title": "Total Profit", "value": f"${format_number(total_profit)}"},
    {"title": "Profit Margin (%)", "value": f"{profit_margin:.1f}%"},
    {"title": "Average Shipment Time", "value": f"{avg_shipment_time:.1f} Days"}
]
